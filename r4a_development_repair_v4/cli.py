"""CLI for the immutable R4-A v4 short-body repair."""
...
from .splitzip import ShortBodyRetryRangeClient
...
def _parser() -> argparse.ArgumentParser:
    ...
    freeze.add_argument("--v3-protocol", type=Path, required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "freeze-development":
        ...
        build_prefreeze_receipt(
            ...
            v3_protocol_path=args.v3_protocol.resolve(),
        ...
        parts = [Part(row["filename"], row["url"], int(row["bytes"]), row["sha256"]) for row in protocol["development_split"]["archives"]]
