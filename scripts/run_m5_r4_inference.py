#!/usr/bin/env python3
"""Run label-blind FoundationPose inference for M5-R4A HB development."""
...
def effective_row(row: dict[str, Any]) -> dict[str, Any]:
    if row.get("record_type") != "m5_r4_development_inference_sample":
        raise ValueError("Unexpected M5-R4 inference manifest row")
    if row.get("sensor_modality") != SENSOR_MODALITY or not row.get("input_available"):
        raise ValueError(f"Invalid M5-R4 inference sample: {row.get('sample_id')}")
    if any(field in row for field in STRIP_EVALUATOR_FIELDS):
        raise ValueError(f"Evaluator field leaked into M5-R4 inference: {row['sample_id']}")
    if bool(row.get("evaluator_label_read")):
        raise ValueError(f"M5-R4 inference row claims an evaluator read: {row['sample_id']}")
    effective = dict(row)
...
def label_blind_result(row: dict[str, Any], estimator: Any) -> dict[str, Any]:
    ...
    result["record_type"] = "m5_r4_development_prediction"
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("M5-R4 development contract is invalid")
    if sha256_file(Path(__file__).resolve()) != contract["code"]["inference"]["sha256"]:
        raise RuntimeError("M5-R4 inference code changed after bundle freeze")
    if sha256_file(manifest_path) != contract["files"]["inference_manifest"]["sha256"]:
        raise RuntimeError("M5-R4 inference manifest hash mismatch")
    rows = load_jsonl(manifest_path)
    expected_count = int(contract["selection_summary"]["inference_sample_count"])
    if len(rows) != expected_count:
        raise ValueError("M5-R4 inference sample count mismatch")
    ids = [str(row["sample_id"]) for row in rows]
    if len(ids) != len(set(ids)):
        raise ValueError("Duplicate M5-R4 inference sample ID")
    for row in rows:
        effective_row(row)
    required_data = [
        data_root / "models" / "models_info.json",
        data_root / "val_primesense",
    ]
    if any(not path.exists() for path in required_data):
        raise FileNotFoundError("HB development dataset structure is incomplete")
    ...
    if not torch.cuda.is_available():
        raise SystemicEnvironmentFailure("CUDA unavailable for M5-R4 inference")
    ...
    if output_path.exists():
        if not args.resume:
            raise RuntimeError("M5-R4 predictions exist; pass --resume only after interruption")
        existing = load_jsonl(output_path)
        if not existing or existing[0].get("record_type") != "m5_r4_inference_metadata":
            raise ValueError("Invalid M5-R4 prediction metadata")
        if existing[0].get("manifest_sha256") != manifest_sha:
            raise ValueError("Existing M5-R4 predictions use another manifest")
        for row in existing[1:]:
            sample_id = str(row["sample_id"])
            if sample_id not in ids or sample_id in completed:
                raise ValueError("Invalid existing M5-R4 prediction identity")
            completed[sample_id] = row
    else:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        metadata = {
            "record_type": "m5_r4_inference_metadata",
    ...
    receipt = {
        "schema_version": SCHEMA_VERSION,
        "stage": "M5-R4A label-blind HB Primesense development inference",
        "status": "complete",
