#!/usr/bin/env python3
"""Evaluate the frozen M5-R5 nearest-anchor family on LM-O development data."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=repo_root / "reports" / "r2" / "m5_r5_development.md",
    )
...
def _config_from_protocol(
...
) -> AnchoredGateConfig | MeasurementFirstConfig:
    ...
    if method == "NEAREST_ANCHORED_OBJECT_CONFIDENCE_GATE":
        return AnchoredGateConfig(**values)
    if method == "MEASUREMENT_FIRST_OBJECT_CONFIDENCE_GATE":
        return MeasurementFirstConfig(**values)
    raise ValueError(f"Unsupported M5 proposed method: {method}")


def _proposed_runtime(protocol: Mapping[str, Any]) -> tuple[Any, Any]:
    method = str(protocol["methods"]["proposed_family"]["method"])
    if method == "NEAREST_ANCHORED_OBJECT_CONFIDENCE_GATE":
        return AnchoredGateObservation, run_nearest_anchored_confidence_gate
    if method == "MEASUREMENT_FIRST_OBJECT_CONFIDENCE_GATE":
        return MeasurementFirstObservation, run_measurement_first_confidence_gate
    raise ValueError(f"Unsupported M5 proposed method: {method}")
...
def _load_and_validate(
    ...
    for key in ("track_manifest", "evaluator_labels"):
        ...
        if sha256_file(path) != str(contract["files"][key]["sha256"]):
            raise RuntimeError(f"M5-R5 {key} hash mismatch")
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if receipt.get("status") != "complete" or bool(
        receipt.get("evaluator_labels_path_read")
    ):
        raise RuntimeError("M5-R5 inference receipt is invalid")
    if receipt.get("protocol_id") != contract["protocol_id"]:
        raise RuntimeError("M5-R5 inference receipt protocol mismatch")
    if sha256_file(predictions_path) != str(receipt["predictions"]["sha256"]):
        raise RuntimeError("M5-R5 predictions hash mismatch")
    prediction_rows = load_jsonl(predictions_path)
    if not prediction_rows or prediction_rows[0].get("record_type") != "m5_r5_inference_metadata":
        raise ValueError("M5-R5 prediction metadata missing")
    if prediction_rows[0].get("protocol_id") != contract["protocol_id"]:
        raise ValueError("M5-R5 prediction metadata protocol mismatch")
    predictions = {
...
def _build_track_sequence(
    ...
    for frame in track["frames"]:
        ...
        if str(label["sample_id"]) != str(frame["sample_id"]):
            raise ValueError(f"M5-R5 label/track mismatch: {track_id}/{replay_index}")
        natural_missing = bool(label["natural_input_missing"])
        if natural_missing == bool(frame["input_available"]):
            raise ValueError(f"M5-R5 availability mismatch: {track_id}/{replay_index}")
        ...
        detail.append(
            {
                ...
                "input_mask_area_fraction": frame["input_mask_area_fraction"],
                "valid_depth_ratio_inside_mask": frame[
...
def _evaluate_track(
    ...
    proposed_config: AnchoredGateConfig | MeasurementFirstConfig,
    proposed_observation_type: Any,
    proposed_runner: Any,
    candidate_id: str,
) -> dict[str, Any]:
    ...
    for index, item in enumerate(sequence.inputs):
        observations.append(
            proposed_observation_type(
                timestamp_s=float(item.timestamp_s),
                anchor_pose=baseline_outputs[index].pose.copy(),
                measurement_pose=(
                    None if item.measurement_pose is None else item.measurement_pose.copy()
                ),
                symmetry=item.symmetry,
                mask_area_fraction=(
                    None
                    if item.measurement_pose is None
                    else float(detail[index]["input_mask_area_fraction"])
                ),
                valid_depth_ratio=(
                    None
                    if item.measurement_pose is None
                    else float(detail[index]["valid_depth_ratio_inside_mask"])
                ),
            )
        )
    proposed_outputs = proposed_runner(observations, proposed_config)
    ...
    missing_anchor_mismatches = int(
        np.sum(
            [
                not np.array_equal(proposed_poses[index], baseline_poses[index])
                for index in np.flatnonzero(natural_missing)
            ]
        )
    )
        ...
        frames.append(
            {
                **base,
                ...
                "proposed_correction_applied": proposed.correction_applied,
                "proposed_correction_gain": proposed.correction_gain,
                "anchor_measurement_disagreement": (
                    proposed.anchor_measurement_disagreement
                ),
    ...
    baseline_missing = (
        float(np.mean(baseline_loss[natural_missing]))
        if np.any(natural_missing)
        else None
    )
    proposed_missing = (
        float(np.mean(proposed_loss[natural_missing]))
        if np.any(natural_missing)
        else None
    )
    ...
    return {
        "record_type": "m5_r5_candidate_track_result",
        ...
        "unsafe_forced_reacquisition_count": sum(
            "reacquir" in str(row["proposed_reason"]) for row in frames
        ),
        "correction_applied_frame_count": sum(
            bool(row["proposed_correction_applied"]) for row in frames
        ),
        ...
        "natural_missing_relative_improvement": (
            ...
            if baseline_missing is not None
            and proposed_missing is not None
...
def _candidate_summary(rows: Sequence[Mapping[str, Any]], candidate_id: str) -> dict[str, Any]:
    ...
    missing_rows = [
        row for row in rows if int(row["natural_missing_frame_count"]) > 0
    ]
    missing_baseline = (
        _macro(missing_rows, "baseline_natural_missing_loss")
        if missing_rows
        else None
    )
    missing_proposed = (
        _macro(missing_rows, "proposed_natural_missing_loss")
        if missing_rows
        else None
    )
    return {
        ...
        "missing_represented_object_count": len(
            {int(row["object_id"]) for row in missing_rows}
        ),
        "natural_missing_macro_object_relative_improvement": (
            (missing_baseline - missing_proposed) / missing_baseline
            if missing_baseline is not None
            and missing_proposed is not None
            and missing_baseline > 0
            else None
        ),
        "nonfinite_output_count": sum(int(row["nonfinite_output_count"]) for row in rows),
        "missing_anchor_mismatch_count": sum(
            int(row["missing_anchor_mismatch_count"]) for row in rows
        ),
        "unsafe_forced_reacquisition_count": sum(
            int(row["unsafe_forced_reacquisition_count"]) for row in rows
        ),
        "correction_applied_frame_count": sum(
            int(row["correction_applied_frame_count"]) for row in rows
        ),
    }
...
def render_report(result: Mapping[str, Any]) -> str:
    ...
    lines = [
        f"# PoseLoop {result['stage_id']} LM-O anchored-confidence development",
        "",
        ...
        f"| Objects represented in missing-frame metric | {observed['missing_represented_object_count']} | >= {thresholds.get('missing_represented_object_count_min', 0)} |",
        f"| Missing-anchor mismatches | {observed['missing_anchor_mismatch_count']} | <= 0 |",
        f"| Unsafe forced reacquisitions | {observed['unsafe_forced_reacquisition_count']} | <= 0 |",
    ...
    for row in result["candidates"]:
        lines.append(
            ...
            f"{row['correction_applied_frame_count']} |"
        )
    lines.extend(
        [
            "",
...
def main() -> None:
    ...
    if result_path.exists() or track_results_path.exists():
        raise FileExistsError("M5-R5 development result already exists")
    contract = json.loads(contract_path.read_text(encoding="utf-8"))
    if contract.get("protocol_id") != args.expected_protocol_id or bool(
        contract.get("sealed_archive_or_label_read")
    ):
        raise RuntimeError("Invalid M5-R5 development contract")
    if sha256_file(Path(__file__).resolve()) != contract["code"]["evaluator"]["sha256"]:
        raise RuntimeError("M5-R5 evaluator changed after bundle freeze")
    core_path = Path(str(contract["code"]["core"]["path"]))
    if sha256_file(core_path) != contract["code"]["core"]["sha256"]:
        raise RuntimeError("M5-R5 core changed after bundle freeze")
    protocol_path = Path(str(contract["protocol"]["path"]))
    if sha256_file(protocol_path) != contract["protocol"]["sha256"]:
        raise RuntimeError("M5-R5 protocol changed after bundle freeze")
    ...
    if sha256_file(models_info_path) != contract["dataset"]["models_info"]["sha256"]:
        raise RuntimeError("M5-R5 models_info hash mismatch")
    ...
    proposed_observation_type, proposed_runner = _proposed_runtime(protocol)
    ...
    for track_index, track in enumerate(tracks, start=1):
        ...
        print(f"{stage_id} track {track_index:02d}/{len(tracks):02d}: {track['track_id']}")
    ...
    for held_out in object_ids:
        ...
        selected = max(
            ...
            key=lambda candidate_id: (
                training_scores[candidate_id], -candidate_order.index(candidate_id)
            ),
        )
        held_rows = [
            row
    ...
    observed = {
        ...
        "missing_represented_object_count": cross_summary[
        ...
        "nonfinite_output_count": cross_summary["nonfinite_output_count"],
        "missing_anchor_mismatch_count": cross_summary[
            "missing_anchor_mismatch_count"
        ],
        "unsafe_forced_reacquisition_count": cross_summary[
    ...
    conditions = {
        ...
        "missing_represented_object_count": observed[
            "missing_represented_object_count"
        ]
        >= int(gate_spec.get("missing_represented_object_count_min", 0)),
        ...
        >= float(gate_spec["cross_fitted_natural_missing_frame_relative_improvement_min"]),
        ...
        <= int(gate_spec["missing_anchor_mismatch_count_max"]),
        "unsafe_forced_reacquisition_count": observed[
            "unsafe_forced_reacquisition_count"
        ]
        <= int(gate_spec["unsafe_forced_reacquisition_count_max"]),
    }
    ...
    result = {
        ...
        "stage_id": stage_id,
        "stage": f"{stage_id} LM-O nearest-anchor object-grouped development",
        "status": (
            f"PASS_{stage_id.replace('-', '_')}_DEVELOPMENT"
            if passed
            else f"FAIL_{stage_id.replace('-', '_')}_DEVELOPMENT"
        ),
        "development_gate": {
            "passed": passed,
        ...
        "proposed_method": protocol["methods"]["proposed_family"]["method"],
