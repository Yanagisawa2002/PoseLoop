"""V1.2 successor of frozen A9; inference algorithms retained, population is dynamic."""
...
SCHEMA = "poseloop.a10-foundationpose-e2e.protocol.v2"
PROTOCOL_ID = "poseloop.pose-accuracy-recovery.a10-foundationpose-e2e.v2"
INPUT_SCHEMA = "poseloop.a10-foundationpose-e2e.input-manifest.v1"
RUN_LOCK_SCHEMA = "poseloop.a10-foundationpose-e2e.run-lock.v1"
RESULT_SCHEMA = "poseloop.a10-foundationpose-e2e.result.v1"
RECEIPT_SCHEMA = "poseloop.a10-foundationpose-e2e.receipt.v1"
EVALUATION_SCHEMA = "poseloop.a10-foundationpose-e2e.evaluation.v1"
...
def load_protocol(path: Path) -> dict[str, Any]:
    ...
    for key in (
        "detector_dataset_manifest_sha256",
        "detector_prediction_manifest_sha256",
        "detector_protocol_sha256",
    ):
    ...
    if (
        ...
        or int(detector.get("expected_ranked_prediction_count", -1)) < expected_count(protocol)
        or not _is_sha256(detector.get("checkpoint_sha256"))
    ...
    from .interfaces import validate_successor_policy
    validate_successor_policy(protocol)
    return protocol


def expected_count(protocol: Mapping[str, Any]) -> int:
    value = protocol["upstream_detector"]["expected_operating_prediction_count"]
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        raise ContractError("Operating count must be a positive integer")
    return value
...
def freeze_inputs(
    *,
    ...
    if (
        ...
        != protocol["dataset"]["detector_dataset_manifest_sha256"]
    ):
    ...
    if (
        ...
        != protocol["dataset"]["detector_prediction_manifest_sha256"]
    ):
        raise ContractError("Frozen A-R9 prediction manifest changed")
    from .interfaces import validate_predictions
    validate_predictions(
        _repo_root() / "protocols/poseloop_pose_accuracy_recovery_real_instance_detector_v1.json",
        dataset_manifest_path, predictions_root,
    )
    upstream_manifest = _read_json(dataset_manifest_path)
    prediction_manifest = _read_json(prediction_manifest_path)
    if (
        prediction_manifest.get("checkpoint_sha256") != protocol["upstream_detector"]["checkpoint_sha256"]
        or int(prediction_manifest.get("frame_count", -1)) != 25
        ...
        != protocol["dataset"]["detector_protocol_sha256"]
    ):
    ...
    if len(items) != int(
        ...
        raise ContractError(f"Expected {expected_count(protocol)} operating predictions, got {len(items)}")
...
def validate_input_manifest(
    ...
    if (
        ...
        or int(manifest.get("item_count", -1)) != expected_count(protocol)
    ...
    if len(items) != expected_count(protocol) or len(set(item_ids)) != expected_count(protocol) or any(not item_id for item_id in item_ids):
...
def _implementation_identity(protocol_path: Path) -> dict[str, Any]:
    root = _repo_root()
    relative_paths = [
        "pose_accuracy_recovery_prep/a10_foundationpose_e2e_v2/__init__.py",
        "pose_accuracy_recovery_prep/a10_foundationpose_e2e_v2/__main__.py",
        "pose_accuracy_recovery_prep/a10_foundationpose_e2e_v2/runtime.py",
        "pose_accuracy_recovery_prep/a10_foundationpose_e2e_v2/interfaces.py",
...
def run_primary(
    *,
    ...
    implementation_paths = sorted(key for key in implementation["files_sha256"] if key != "generated-pose-protocol")
...
def validate_primary_evidence(primary_root: Path, manifest_path: Path, protocol_path: Path) -> None:
    """Bind completed predictions to the exact frozen input before label access."""
    manifest = validate_input_manifest(manifest_path, protocol_path, verify_assets=False)
    lock = _read_json(primary_root / "run-lock.json")
    completion = _read_json(primary_root / "completion-receipt.json")
    if (lock.get("manifest_sha256") != _sha256_file(manifest_path)
        or lock.get("protocol_sha256") != _sha256_file(protocol_path)
        or lock.get("run_lock_sha256") != _canonical_sha256(_without_lock(lock, "run_lock_sha256"))
        or completion.get("run_lock_sha256") != lock["run_lock_sha256"]
        or completion.get("item_count") != manifest["item_count"]
        or completion.get("status") != "COMPLETE"
        or completion.get("predictions_sha256") != _sha256_file(primary_root / "predictions.jsonl")):
        raise ContractError("Primary evidence does not bind the current input")
    expected = {str(item["item_id"]) for item in manifest["items"]}
    rows = _load_existing_results(primary_root / "predictions.jsonl", lock["run_lock_sha256"], expected)
    if set(rows) != expected:
        raise ContractError("Primary evidence has incomplete coverage")
...
def evaluate(
    *,
    ...
    validate_primary_evidence(primary_root, manifest_path, protocol_path)
    completion = _read_json(primary_root / "completion-receipt.json")
    ...
    if len(rows) != expected_count(protocol) + 1 or rows[0].get("record_type") != "metadata":
        raise ContractError("Primary prediction coverage changed")
    result_by_id = {str(row["item_id"]): row for row in rows[1:]}
    if len(result_by_id) != expected_count(protocol) or set(result_by_id) != {str(item["item_id"]) for item in manifest["items"]}:
    ...
    runtime_completion_fraction = runtime_success / expected_count(protocol)
    output_root.mkdir(parents=True)
    ...
    result = {
        ...
        if passed
        else "NO_GO_DEVELOPMENT_THRESHOLDS",
        "protocol_sha256": _sha256_file(protocol_path),
        "manifest_sha256": _sha256_file(manifest_path),
        ...
        "prediction_count": expected_count(protocol),
...
| Metric | Result |
| --- | ---: |
| Runtime completion | {runtime_success}/{expected_count(protocol)} ({runtime_completion_fraction:.4f}) |
...
def package_evidence(
    *,
    ...
    if result.get("schema_version") != EVALUATION_SCHEMA:
        raise ContractError("Expected v1.2 evaluation; negative results must also be preserved")
