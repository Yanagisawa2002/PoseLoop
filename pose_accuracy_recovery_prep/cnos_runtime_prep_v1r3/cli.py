"""CLI for the inert A-R3 CNOS descriptor-renderer repair route."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m pose_accuracy_recovery_prep.cnos_runtime_prep_v1r3",
        description=(
            "Validate and run the A-R3 hash-locked thin wrapper around the "
            "pinned official CNOS CAD renderer. This CLI never runs FastSAM, "
            "DINOv2 matching, FoundationPose, or an evaluator."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        route = load_json_object(args.route, "CNOS R3 route")
        if args.command == "validate-route":
            _emit(
                validate_route(route, repository_root=args.repository_root),
                args.output,
            )
            return 0
        request = load_json_object(args.request, "CNOS R3 render request")
